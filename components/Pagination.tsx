	...
	return (
		<div className="text-xs md:text-md flex p-2 bg-slate-100 align-items-start rounded-md">
			...
			{pages.map((page) => {
				if (page !== null) {
					return (
						<div className={`text-center p-2 cursor-pointer font-bold ${page.index == currentPageIndex ? 'bg-white rounded-full' : ''}`} key={`pagination-${page.index}`} onClick={(event) => changePageIndex(page.index)}>
							{page.number}
						</div>
					)
				}
					...
					return (
						<div className="text-center p-2 font-bold">...</div>
					)
				}
			})}
